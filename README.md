...
원하는 요소를 핀포인트로 집어내기 위해서는 셀렉터를 사용할 수 있습니다.
...
명령어의 대괄호 `[ ]` 옵션 내부를 상세하게 지정하여 쿼리문을 간단하게 입력할 수 있습니다.
- **`#w`**: width (너비)
- **`#h`**: height (높이)
- **`#s`**: scale (축척)
- **`#c`**: color (색상)
- **`#scale`**: width, height, scale을 통칭하는 속성.
...
`; move $cap >> $img.|` 라고 명령하면, 1번 캡션을 1번 이미지에 붙이지 않고, **"이 캡션과 이 이미지가 같은 figure 환경(부모) 안에 들어있는가?"**를 먼저 검증하여 같은 가족(Scope)끼리만 짝을 지어 처리합니다.
...
**도전**: 수백 개의 그림 중 캡션이 누락된 그림들을 찾아, 중앙 정렬을 해주고 "[TODO: 캡션 작성]" 이라는 문구를 넣습니다.
```
