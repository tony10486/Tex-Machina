...
## Features
### Calculation
NumPy, SymPy, SciPy 패키지를 이용한 수식 계산 기능입니다. `calc` 명령어를 통해 사용할 수 있습니다.

### 상세 예시 및 활용 
`calc` 기능은 문서 내의 LaTeX 수식을 드래그(선택)한 후 명령어를 입력하거나, 명령어 뒤에 직접 수식을 입력하여 사용할 수 있습니다.
- 미분/적분: 주어진 함수의 미분과 적분을 수행합니다.
...
- RREF 및 행렬식: 주어진 행렬을 RREF로 변환하거나 행렬식을 계산합니다.
  - 사용: 3x3 행렬 선택 -> `calc > rref, calc > det`
- 대수적인 방정식의 풀이: 여러 가지 방정식의 해를 구 합니다.
...
*   3D 시각화 (Three.js/WebGL2): 복잡한 3D 곡면이나 함수를 VS Code 내부 웹뷰 패널에서 마우스로 돌려보며 확인하고, 이를 PDF나 PNG 형태의 그림 파일로 즉시 내보낼 수 있습니다. 로컬 번들 렌더러(CDN 없음, 오프라인 동작) + `clip=exact` 옵션 시 정확한 레벨셋 클립(특이점 캡)을 지원합니다.
