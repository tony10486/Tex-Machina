...
### 고급 매크로 및 컨텍스트 인지
단순한 텍스트 대치를 넘어, 현재 커서가 위치한 상황(수식 내부/외부 등)에 따라 다르게 동작하는 매크로를 지원합니다.
...
### 스마트 검색 및 주입 (Smart Search & Inject)
특정 환경 내의 특정 명령어만 찾아 옵션을 일괄적으로 수정하거나 텍스트를 주입합니다.

*   정교한 쿼리: `('figure > \includegraphics:not([alt])').inject('alt={}')`와 같은 CSS 선택자 스타일의 쿼리를 사용하여, figure 환경 내부에 있으면서 alt 속성이 없는 모든 includegraphics에 alt 옵션을 추가합니다.
