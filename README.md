...
## Macro
이 확장 프로그램에서 가장 강력한 기능입니다! 그래서 따로 항목으로 빼놨습니다. 
...
작은따옴표 `' '`는 환경, 명령어, 속성 등을 지칭할 때 사용하며, 공백이나 특수 문자가 포함된 경우 이를 하나로 묶어주는 역할을 합니다. 다만, **단순한 명령어(`\includegraphics`), 환경(`figure`), 태그(`@img`) 등은 작은따옴표 없이도 정상적으로 인식**됩니다. 조건, 연산, 메모리, 반복문의 경우에는 작은따옴표를 사용하지 않습니다. 
...
- 1항 구조 명령어
  1. find : 대상을 찾습니다. 기본 옵션은 파일 내 모든 내용에 대해 검색을 수행하는 옵션 :all 입니다. 기본적으로 현재 작업하는 파일 내에 대해 탐색을 수행합니다. 
      > find 명령어는 암묵적으로 생략이 가능합니다. 즉, `?find '\includegraphics'`는 `? \includegraphics` 또는 `? '\includegraphics'`로 쓸 수 있습니다. 특히 태그를 사용할 경우 `? @img`와 같이 간결하게 표현할 수 있습니다.
...
명령어가 작업을 수행할 대상과, 수행할 내용 등의 구조적 대상은 필요한 경우 작은따옴표로 감쌀 수 있습니다.
...
#### 연산자
1. 계층 및 탐색 연산자 : 계층 및 탐색 연산자는 대상의 속성을 구체적으로 지정하는 역할을 합니다. 복잡한 구조를 탐색할 때 작은따옴표를 사용하여 하나의 경로로 묶을 수 있지만, 연산자 앞뒤로 공백을 두어 구분하는 방식도 지원합니다.
...
- 텍스트 : 위 두 유형을 제외한 LaTeX의 요소입니다. 이는 큰따옴표로 묶어야 합니다. 예를 들어 `"Birds"` 가 있습니다. 
  - 원칙적으로 대상이나 작업은 작은따옴표(`' '`)로 감싸는 것이 명시적이지만, 공백이 없는 명령어(`\includegraphics`)나 태그(`@img`) 등은 생략이 가능합니다.
  - 큰따옴표(`" "`)로 감싸진 경우, 파서는 이를 텍스트로 인식하며 필요 시 자동으로 `'"텍스트"'`와 같이 감싸진 것으로 간주하여 처리합니다. 이는 쿼리 작성을 더욱 빠르고 직관적으로 만듭니다.

### 사용 예시
- **이미지 경로 일괄 변경**: 모든 이미지의 경로 앞에 `images/`를 붙이고 싶을 때 (작은따옴표 생략 가능)
  ```
  ? @img{*} >> "images/*"
  ```
- **특정 환경 내 커서 위치**: 파일 내 모든 `\includegraphics` 명령어 중 대괄호가 있는 `\includegraphics` 명령어에 대해 대괄호 속 커서를 위치시킵니다. 
