...
export function registerSmartNewline(context: vscode.ExtensionContext) {
    const disposable = vscode.commands.registerCommand('tex-machina.smartNewline', async () => {
        ...
        // Detect leading inline content before \begin{env} on the same line
        let leadingContent = '';
        if (isRightAfterBegin) {
            const beginIdx = textBeforeCursor.indexOf('\\begin');
            if (beginIdx > 0) {
                const beforeBegin = textBeforeCursor.substring(0, beginIdx);
                if (beforeBegin.trim().length > 0) {
                    leadingContent = beforeBegin.trimEnd();
                }
            }
        }
        ...
        if (isRightAfterBegin || cursorBeforeEndOnEndLine) {
            ...
            if (isRightAfterBegin && restIsOnlyEnd) {
                ...
                const beginCmd = textBeforeCursor.substring(textBeforeCursor.indexOf('\\begin'));

                if (leadingContent) {
                    // Split leading content to its own line, then expand the environment
                    await editor.edit(editBuilder => {
                        editBuilder.replace(
                    ...
                    await editor.edit(editBuilder => {
                        ...
                        if (nextLineIsEnd) {
                            ...
                            if (endIndent !== beginIndent) {
                                editBuilder.replace(
                                    new vscode.Range(nextLineNum + 1, 0, nextLineNum + 1, endIndent.length),
                                    beginIndent
                                );
                            }
                        }
