...
- 스마트 아래첨자 기능에서 문제가 있음. v_1, 을 할 떄 v_{1,}로 표현되어 치기가 불편한 문제가 있음. 쉼표가 있을 때는 넘어가도록 수정이 필요함. 추가로 v^n$ 을 치면 끝나야 하는데 이 또한 v^{n$}가 되어 불편함. 즉, 쉼표, 괄호 닫기, 수식 닫기 등에 대해서는 해당 기능이 작동하지 않도록 바꿔야 함 
- 웹뷰에서 제대로 레퍼런스가 보이지 않는 문제가 있음
- matrix 입력 시 들여쓰기가 되도록 고쳐야 함
\begin{gather}
    \mathcal{M}(T) 
    \begin{bmatrix}
2 & 1 & 0 \\
