...
## 개선이 핑요한 사항
- calc 기능에서 정적분을 수행할 때, 기호에 있는 적분 구간을 보고 자동으로 적분구간을 설정하도록 해야 함. 
...
- 단진자 미방을 풀지 못하는가?
- calc 기능에서 적분 해결에 문제가 있음 
    >ex : \int \sin (x) dx = - \sin{\left(x \right)} (정답은 \cos (x) + C)

- calc 기능에서 미분 해결에도 문제가 있음
    >ex : \frac{\sinh (x) \log (x) }{e^{x}} = - \frac{e^{- x} x \log{\left(x \right)} \sinh{\left(x \right)}}{e} 