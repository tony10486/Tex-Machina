...
## 추가 예정 기능 
- 자동 완성 이스케이프 : 자동 완성 모드를 끄고 싶다면 esc(윈도우), 또는 escape(mac)을 눌러 자동 완성을 이스케이프 할 수 있도록 함
...
- 다이어크리틱 입력 : 글자 a를 치고 Alt + ^ 를 누르면 \hat{a}로, Alt + ~ 를 누르면 \tilde{a}로, Alt + . 을 누르면 \dot{a}로 커서를 감싸며 즉시 변환됩니다.
