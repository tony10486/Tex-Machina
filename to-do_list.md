...
- [x] 수식 자동 완성이 되어 있어도, 이미 완전한 명령어에 대해서는 자동완성이 이루어지지 않도록 해야 함.
    > Ex : calc를 입력 후 엔터를 치면 calc 명령어만 입력되어야 하지만, calc > 로 자동완성이 되는 문제가 있음 (해결됨)
- 모든 기능을 gemini cli에게 테스트를 하라 시키자.
